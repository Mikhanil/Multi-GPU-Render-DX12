                             ...
                             UINT passCount, UINT materialCount)
{
    ...
    PrimeMbConstantUploadBuffer = (std::make_shared<ConstantUploadBuffer<MBConstants>>(
        primeDevices, 1, primeDevices->GetName() + L" MB Path Data Buffer"));

    SecondMbConstantUploadBuffer = (std::make_shared<ConstantUploadBuffer<MBConstants>>(
        secondDevice, 1, secondDevice->GetName() + L" MB Path Data Buffer"));
