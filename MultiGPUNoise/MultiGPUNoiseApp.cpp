...
MultiGPUNoiseApp::MultiGPUNoiseApp(HINSTANCE hInstance): D3DApp(hInstance)
{
	mSceneBounds.Center = Vector3(0.0f, 0.0f, 0.0f);
	mSceneBounds.Radius = 200;
}

MultiGPUNoiseApp::~MultiGPUNoiseApp()
{
}

void MultiGPUNoiseApp::Update(const GameTimer& gt)
{
...
void MultiGPUNoiseApp::PopulateShadowMapCommands(std::shared_ptr<GCommandList> cmdList)
{
...
void MultiGPUNoiseApp::PopulateNormalMapCommands(std::shared_ptr<GCommandList> cmdList)
{
	//Draw Normals
	{
...
void MultiGPUNoiseApp::PopulateAmbientMapCommands(std::shared_ptr<GCommandList> cmdList)
{
	//Draw Ambient
	{
		cmdList->SetDescriptorsHeap(&srvTexturesMemory);
...
void MultiGPUNoiseApp::PopulateForwardPathCommands(std::shared_ptr<GCommandList> cmdList)
{
...
void MultiGPUNoiseApp::PopulateDrawCommands(std::shared_ptr<GCommandList> cmdList,
...
void MultiGPUNoiseApp::PopulateInitRenderTarget(std::shared_ptr<GCommandList> cmdList, GTexture& renderTarget,
...
void MultiGPUNoiseApp::PopulateDrawFullQuadTexture(std::shared_ptr<GCommandList> cmdList,
...
bool MultiGPUNoiseApp::Initialize()
{
	InitDevices();
	InitMainWindow();
...
void MultiGPUNoiseApp::InitDevices()
{
...
void MultiGPUNoiseApp::InitRootSignature()
{
...
void MultiGPUNoiseApp::InitPipeLineResource()
{
	defaultInputLayout =
	{
		{
...
void MultiGPUNoiseApp::CreateMaterials()
{
...
void MultiGPUNoiseApp::InitSRVMemoryAndMaterials()
{
	srvTexturesMemory =
...
void MultiGPUNoiseApp::InitRenderPaths()
{
...
void MultiGPUNoiseApp::LoadStudyTexture()
{
...
void MultiGPUNoiseApp::LoadModels()
{
	auto queue = primeDevice->GetCommandQueue(D3D12_COMMAND_LIST_TYPE_COPY);
	const auto cmdList = queue->GetCommandList();
...
void MultiGPUNoiseApp::MipMasGenerate()
{
	try
	{
...
void MultiGPUNoiseApp::CreateGO()
{
	logQueue.Push(std::wstring(L"\nStart Create GO"));
...
void MultiGPUNoiseApp::CalculateFrameStats()
{
...
void MultiGPUNoiseApp::UpdateMaterials()
{
	{
...
void MultiGPUNoiseApp::UpdateShadowTransform(const GameTimer& gt)
{
...
void MultiGPUNoiseApp::UpdateShadowPassCB(const GameTimer& gt)
{
...
void MultiGPUNoiseApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void MultiGPUNoiseApp::UpdateSsaoCB(const GameTimer& gt)
{
...
bool MultiGPUNoiseApp::InitMainWindow()
{
...
void MultiGPUNoiseApp::OnResize()
{
	D3DApp::OnResize();
...
void MultiGPUNoiseApp::Flush()
{
	primeDevice->Flush();
	secondDevice->Flush();
}

LRESULT MultiGPUNoiseApp::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_INPUT:
