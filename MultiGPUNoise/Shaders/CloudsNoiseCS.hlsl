...
void CS( uint3 DTid : SV_DispatchThreadID )
{
    ...
    float4 cloudColor = generatorBuffer.CloudColor * clamp((generatorBuffer.CloudDark + generatorBuffer.CloudLight * c), 0.0, 1.0);
   
    f = generatorBuffer.CloudCover + generatorBuffer.CloudAlpha * f * r;
    
    float4 result = lerp(skyColor, clamp(generatorBuffer.SkyTint * skyColor + cloudColor, 0.0, 1.0), clamp(f + c, 0.0, 1.0));
