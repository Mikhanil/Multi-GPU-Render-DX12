...
SplitFrameResource::SplitFrameResource(std::shared_ptr<GDevice>* devices, UINT deviceCount, UINT passCount, UINT materialCount)
{
	...
	{
		PassConstantUploadBuffers.push_back(std::make_shared<ConstantUploadBuffer<PassConstants>>(devices[i], passCount, devices[i]->GetName() + L" Forward Path Data"));
		SsaoConstantUploadBuffers.push_back(std::make_shared<ConstantUploadBuffer<SsaoConstants>>(devices[i], 1, devices[i]->GetName() + L" SSAO Path Data"));
		MaterialBuffers.push_back(std::make_shared<StructuredUploadBuffer<MaterialConstants>>(devices[i], materialCount, devices[i]->GetName() + L" Materials Data"));
