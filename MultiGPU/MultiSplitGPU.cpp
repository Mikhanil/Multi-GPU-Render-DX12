...
void MultiSplitGPU::InitPipeLineResource()
{
	...
	for (int i = 0; i < GraphicAdapterCount; ++i)
	{
		...
		ambientPaths[i]->SetPSOs(*defaultPipelineResources[i].GetPSO(RenderMode::Ssao), *defaultPipelineResources[i].GetPSO(RenderMode::SsaoBlur));
...
void MultiSplitGPU::AddMultiDeviceOpaqueRenderComponent(GameObject* object, std::wstring modelName, RenderMode::Mode psoType)
{
	for (int i = 0; i < GraphicAdapterCount; ++i)
	{
...
void MultiSplitGPU::PopulateShadowMapCommands(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList)
{
	//Draw Shadow Map
	{
		...
		cmdList->SetRootConstantBufferView(StandardShaderSlot::CameraData, *currentFrameResource->PassConstantUploadBuffers[adapterIndex], 1);
...
void MultiSplitGPU::PopulateAmbientMapCommands(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList)
{
	//Draw Ambient
	{
		...
		ambientPaths[adapterIndex]->ComputeSsao(cmdList, currentFrameResource->SsaoConstantUploadBuffers[adapterIndex], 3);
	}
}
...
void MultiSplitGPU::PopulateForwardPathCommands(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList)
{
	//Forward Path with SSAA
	{
		...
		cmdList->
			SetRootConstantBufferView(StandardShaderSlot::CameraData, *currentFrameResource->PassConstantUploadBuffers[adapterIndex]);
...
void MultiSplitGPU::PopulateDrawCommands(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList, RenderMode::Mode type)
{
