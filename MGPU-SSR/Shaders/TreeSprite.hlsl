...
float4 PS(GeoOut pin) : SV_Target
{
    ...
    directLight += shadowFactor[0] *
        ComputeDirectionalLight(worldBuffer.DirectionalLight, mat, pin.NormalW, toEyeW);
    ...
    for (uint pointLightIndex = 0; pointLightIndex < worldBuffer.PointLightCount; ++pointLightIndex)
    {
        ...
        float falloffEndSq = pointLight.FalloffEnd * pointLight.FalloffEnd;
        if (dot(toLight, toLight) > falloffEndSq)
            continue;
    ...
    for (uint spotLightIndex = 0; spotLightIndex < worldBuffer.SpotLightCount; ++spotLightIndex)
    {
        ...
        float falloffEndSq = spotLight.FalloffEnd * spotLight.FalloffEnd;
        float distanceSq = dot(toLight, toLight);
        if (distanceSq > falloffEndSq)
            continue;
