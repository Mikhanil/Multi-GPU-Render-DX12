...
void HybridGrassApp::EnablePerformanceSweepMode(int warmupSeconds, int sampleSeconds)
{
    ...
    perfScenarioAggregates.assign(
        perfScenarios.size(), std::array<PerfAggregate, PerfCaseCount>{});
}
...
void HybridGrassApp::PopulateNormalMapCommands(const std::shared_ptr<GCommandList>& cmdList)
{
    //Draw Normals
    {
        ...
        for (auto* emitter : crossGrassEmitters)
        {
            emitter->SetWorldConstantsBuffer(currentFrameResource->PrimePassConstantUploadBuffer.get());
            emitter->DrawNormals(cmdList);
        }
...
void HybridGrassApp::Draw(const GameTimer& gt)
{
...

    {
        ...
        for (auto* emitter : sceneEmitters)
            emitter->Dispatch(cmdList);
...
void HybridGrassApp::InitDevices()
{
    ...
    grassRenderPath = performanceTestMode ? GrassRenderPath::SingleGeometry : GrassRenderPath::SingleExpanded;
...
void HybridGrassApp::InitRenderPaths()
{
    ...
    antiAliasingPrimePath = (std::make_shared<SSAA>(primeDevice, performanceTestMode ? 1 : 4, MainWindow->GetClientWidth(),
...
void HybridGrassApp::LoadModels()
{
    ...
    const auto load = [this, &cmdList](const wchar_t* name, const char* path)
    {
        models[name] = assets->CreateModelFromFile(cmdList, path);
    };
...
void HybridGrassApp::CalculateFrameStats()
{
    ...
    if (performanceTestMode)
    {
        ...
        {
            Flush();
            cameraOrbitTransform->SetLocalMatrix(cameraOrbitStartMatrix);
            ...
            const int stagesPerSsaa = static_cast<int>(perfScenarios.size()) * PerfRenderPathCount;
            const int scenarioIndex = performanceSweepMode
                                          ? (perfCurrentStage % stagesPerSsaa) / PerfRenderPathCount
                                          : 0;
            ...
            const UINT ssaaMultiplier = 1 + (performanceSweepMode
                                                   ? perfCurrentStage / stagesPerSsaa
                                                   : (perfCurrentStage % PerfCaseCount) / PerfRenderPathCount);
            antiAliasingPrimePath->SetMultiplier(
                ssaaMultiplier, MainWindow->GetClientWidth(), MainWindow->GetClientHeight());
        ...
        if (stageElapsed >= warmupEnd && stageElapsed < stageEnd)
        {
            ...
            {
                const int stagesPerSsaa = static_cast<int>(perfScenarios.size()) * PerfRenderPathCount;
                const int scenarioIndex = (perfCurrentStage % stagesPerSsaa) / PerfRenderPathCount;
                const int ssaaIndex = perfCurrentStage / stagesPerSsaa;
                const int caseIndex = ssaaIndex * PerfRenderPathCount +
                    perfCurrentStage % PerfRenderPathCount;
                if (scenarioIndex >= 0 && scenarioIndex < static_cast<int>(perfScenarioAggregates.size()))
                {
                    aggregate = &perfScenarioAggregates[scenarioIndex][caseIndex];
                }
            }
            else
            {
        ...
        if (stageElapsed >= stageEnd)
        {
            ...
            const int stageCount = performanceSweepMode
                                       ? static_cast<int>(perfScenarios.size() * PerfCaseCount)
                                       : PerfCaseCount;
            if (performanceSweepMode)
            {
                const int stagesPerSsaa = static_cast<int>(perfScenarios.size()) * PerfRenderPathCount;
                if (perfCurrentStage % stagesPerSsaa == 0)
                {
                    const int completedSsaaIndex = perfCurrentStage / stagesPerSsaa - 1;
                    Flush();
                    WritePerformanceSweepResult(completedSsaaIndex);
                }
            }
        ...
        const int stagesPerSsaa = static_cast<int>(perfScenarios.size()) * PerfRenderPathCount;
        const int ssaaIndex = performanceSweepMode
                                  ? perfCurrentStage / stagesPerSsaa
                                  : (perfCurrentStage % PerfCaseCount) / PerfRenderPathCount;
        title += L" | SSAA X" + std::to_wstring(1 + ssaaIndex);
        if (performanceSweepMode)
        {
            const int scenarioIndex = (perfCurrentStage % stagesPerSsaa) / PerfRenderPathCount;
...
std::wstring HybridGrassApp::PerformanceResultPath() const
{
    std::wstring name = primeDevice->GetName();
    if (HaveTwoHardwareAdapters)
        name += L"+" + secondDevice->GetName();
    for (auto& ch : name)
        if (ch < 32 || std::wstring_view(L"<>:\"/\\|?*").find(ch) != std::wstring_view::npos)
            ch = L'_';
    name += performanceSweepMode ? L"-result.csv" : L"-test-result.csv";
...
void HybridGrassApp::WritePerformanceTestResults()
{
    ...
    file << L"mode;ssaa_multiplier;samples;avg_fps;min_fps;max_fps;avg_prime_render_ticks;avg_second_render_ticks;avg_prime_compute_ticks;avg_second_compute_ticks\n";
    auto writeRow = [&](const wchar_t* mode, const int ssaaMultiplier, const PerfAggregate& a)
    {
        assert(a.samples > 0); // Every SSAA/render-path case must reach the sampling phase.
        ...
        file << mode << L";" << ssaaMultiplier << L";" << a.samples << L";"
    ...
    {
        writeRow(PerformanceRenderPathCsvName(kPerformanceRenderPaths[caseIndex % PerfRenderPathCount]),
                 1 + caseIndex / PerfRenderPathCount, perfAggregates[caseIndex]);
    }
    file.close();
    logQueue.Push(L"\nPerformance test saved: " + perfResultPath);
}

void HybridGrassApp::WritePerformanceSweepResult(const int ssaaIndex)
{
    if (ssaaIndex < 0 || ssaaIndex >= PerfSsaaCount)
        return;
...
void HybridGrassApp::UpdateShadowTransform(const GameTimer& gt)
{
    ...
    Vector3 lightPos = mSceneBounds.Center - 2.0f * mSceneBounds.Radius * lightDir;
...
void HybridGrassApp::GetGrassWindFieldExtents(float& outCenterX, float& outCenterZ, float& outHalfExtent) const
{
    outCenterX = 0.0f;
    outCenterZ = 0.0f;
    outHalfExtent = std::max(1.0f, grassWorldSize * 0.5f);
...
void HybridGrassApp::DrawImGui(const std::shared_ptr<GCommandList>& cmdList)
{
    ...
    if (performanceTestMode)
    {
        // Same demo and animated plot workload as the MB sample, using our existing backend.
        ...
        float samples[100];
        for (int i = 0; i < 100; ++i)
            samples[i] = std::sin(static_cast<float>(i) * 0.2f + static_cast<float>(ImGui::GetTime()) * 1.5f);
        ImGui::PlotLines("Samples", samples, 100);
        const int stagesPerSsaa = static_cast<int>(perfScenarios.size()) * PerfRenderPathCount;
        const int ssaaIndex = performanceSweepMode
                                  ? perfCurrentStage / stagesPerSsaa
                                  : (perfCurrentStage % PerfCaseCount) / PerfRenderPathCount;
        ImGui::Text("Grass stage: %d | SSAA x%d", perfCurrentStage + 1,
                    1 + ssaaIndex);
        ImGui::End();
        ImGui::Render();
