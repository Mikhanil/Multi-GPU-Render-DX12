...
public:
    ...
    bool IsWindFluidGpuReady() const
    {
        return renderPath_ == RenderPath::SingleExpanded
                   ? singleWindFluid_.IsInitialized()
                   : windFluid.IsInitialized();
    }
    bool IsExpandedComputeActive() const { return renderPath_ != RenderPath::SingleGeometry; }
    bool IsCrossAdapterSharedComputeActive() const { return renderPath_ == RenderPath::MultiExpanded; }
    ...
    const PEPEngine::Graphics::WindFluidSimulator& GetWindFluid() const
    {
...
private:
    ...
    void CreateBuffers();
    void CreateSingleExpandBuffers();
    void DescriptorInitialize();
    void DescriptorInitializeExpandedDraw();
    void DescriptorInitializeSingleExpand();
    void DescriptorInitializeSingleExpandedDraw();
    ...
    std::shared_ptr<GBuffer> singleGrassBuffer_;
    std::shared_ptr<GBuffer> singleExpandedVertexBuffer_;
    std::shared_ptr<GBuffer> singleVisibleVertexCountBuffer_;
    std::shared_ptr<GBuffer> singleGrassEmitterCb_;
    std::shared_ptr<GBuffer> singleGrassCullCb_;
    GDescriptor singleComputeDescriptors_;
    GDescriptor singleExpandDescriptors_;
    GDescriptor singleExpandedDrawDescriptors_;
    std::shared_ptr<ComputePSO> singleGeneratePSO_;
    std::shared_ptr<ComputePSO> singleExpandPSO_;
    std::shared_ptr<GRootSignature> singleComputeRS_;
    std::shared_ptr<GRootSignature> singleExpandRS_;
    ...
    bool singleWindFluidInitGiveUp_{false};
