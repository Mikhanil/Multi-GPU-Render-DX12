...
void CrossAdapterGrassEmitter::InitSingleExpandPSO()
{
    CD3DX12_DESCRIPTOR_RANGE generateUavRange;
    generateUavRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 0);

    singleComputeRS_ = std::make_shared<GRootSignature>();
    singleComputeRS_->AddConstantBufferParameter(0);
    singleComputeRS_->AddDescriptorParameter(&generateUavRange, 1);
    singleComputeRS_->Initialize(primeDevice, false, D3D12_ROOT_SIGNATURE_FLAG_NONE);

    auto generateShader = std::make_shared<GShader>(
        L"Shaders\\ComputeGrass.hlsl", ComputeShader, nullptr, "CS", "cs_5_1");
    generateShader->LoadAndCompile();

    singleGeneratePSO_ = std::make_shared<ComputePSO>();
    singleGeneratePSO_->SetRootSignature(*singleComputeRS_);
    singleGeneratePSO_->SetShader(generateShader.get());
    singleGeneratePSO_->Initialize(primeDevice);
    ...
    singleExpandRS_ = std::make_shared<GRootSignature>();
    singleExpandRS_->AddConstantBufferParameter(0);
    singleExpandRS_->AddConstantBufferParameter(1);
    singleExpandRS_->AddDescriptorParameter(&expandInputRange, 1);
    singleExpandRS_->AddDescriptorParameter(&expandOutputRanges[0], 1);
    singleExpandRS_->AddDescriptorParameter(&expandOutputRanges[1], 1);
    CD3DX12_STATIC_SAMPLER_DESC expandSampler(
        0,
        D3D12_FILTER_MIN_MAG_MIP_LINEAR,
        D3D12_TEXTURE_ADDRESS_MODE_CLAMP,
        D3D12_TEXTURE_ADDRESS_MODE_CLAMP,
        D3D12_TEXTURE_ADDRESS_MODE_CLAMP);
    singleExpandRS_->AddStaticSampler(expandSampler);
    singleExpandRS_->Initialize(primeDevice, false, D3D12_ROOT_SIGNATURE_FLAG_NONE);

    auto expandShader = std::make_shared<GShader>(
        L"Shaders\\ComputeGrass.hlsl", ComputeShader, kComputeGrassExpandDefines,
        "CS_ExpandGrassToVertices", "cs_5_1");
    expandShader->LoadAndCompile();

    singleExpandPSO_ = std::make_shared<ComputePSO>();
    singleExpandPSO_->SetRootSignature(*singleExpandRS_);
    singleExpandPSO_->SetShader(expandShader.get());
    singleExpandPSO_->Initialize(primeDevice);

    InitExpandedDrawPSO();
}

void CrossAdapterGrassEmitter::InitExpandedDrawPSO()
{
    if (expandedDrawPSO)
        return;
...
void CrossAdapterGrassEmitter::CreateSingleExpandBuffers()
{
    if (singleGrassBuffer_)
    {
        singleGrassBuffer_->Reset();
        singleGrassBuffer_.reset();
    }
    if (singleExpandedVertexBuffer_)
    {
        singleExpandedVertexBuffer_->Reset();
        singleExpandedVertexBuffer_.reset();
    }
    if (singleVisibleVertexCountBuffer_)
    {
        singleVisibleVertexCountBuffer_->Reset();
        singleVisibleVertexCountBuffer_.reset();
    }

    singleGrassBuffer_ = std::make_shared<GBuffer>(
        primeDevice,
        sizeof(GrassData),
        emitterData.GrassCount,
        L"Single GPU Grass Buffer",
        D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    singleExpandedVertexBuffer_ = std::make_shared<GBuffer>(
        primeDevice,
        sizeof(GrassRenderVertex),
        emitterData.GrassCount * kMaxVerticesPerBlade,
        L"Single GPU Expanded Grass Vertex Buffer",
        D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    singleVisibleVertexCountBuffer_ = std::make_shared<GBuffer>(
        primeDevice,
        sizeof(uint32_t),
        1,
        L"Single GPU Visible Grass Vertex Count",
        D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
}
...
void CrossAdapterGrassEmitter::DescriptorInitializeSingleExpand()
{
    singleComputeDescriptors_ =
        primeDevice->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 1);
    singleExpandDescriptors_ = primeDevice->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 4);

    D3D12_UNORDERED_ACCESS_VIEW_DESC grassUav{};
    grassUav.Format = DXGI_FORMAT_UNKNOWN;
    grassUav.ViewDimension = D3D12_UAV_DIMENSION_BUFFER;
    grassUav.Buffer.FirstElement = 0;
    grassUav.Buffer.NumElements = emitterData.GrassCount;
    grassUav.Buffer.StructureByteStride = sizeof(GrassData);
    grassUav.Buffer.Flags = D3D12_BUFFER_UAV_FLAG_NONE;
    singleGrassBuffer_->CreateUnorderedAccessView(&grassUav, &singleComputeDescriptors_, 0);

    D3D12_SHADER_RESOURCE_VIEW_DESC grassSrv{};
    grassSrv.Format = DXGI_FORMAT_UNKNOWN;
    grassSrv.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
    grassSrv.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
    grassSrv.Buffer.FirstElement = 0;
    grassSrv.Buffer.NumElements = emitterData.GrassCount;
    grassSrv.Buffer.StructureByteStride = sizeof(GrassData);
    ...
    expandedUav.ViewDimension = D3D12_UAV_DIMENSION_BUFFER;
    expandedUav.Buffer.FirstElement = 0;
    expandedUav.Buffer.NumElements = emitterData.GrassCount * kMaxVerticesPerBlade;
    expandedUav.Buffer.StructureByteStride = sizeof(GrassRenderVertex);
    expandedUav.Buffer.Flags = D3D12_BUFFER_UAV_FLAG_NONE;
    singleExpandedVertexBuffer_->CreateUnorderedAccessView(&expandedUav, &singleExpandDescriptors_, 2);
    ...
    counterUav.Buffer.Flags = D3D12_BUFFER_UAV_FLAG_NONE;
    singleVisibleVertexCountBuffer_->CreateUnorderedAccessView(&counterUav, &singleExpandDescriptors_, 3);
}

void CrossAdapterGrassEmitter::DescriptorInitializeSingleExpandedDraw()
{
    singleExpandedDrawDescriptors_ =
        primeDevice->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 3);
    ...
    if (GTexture* grassTex = primeGrassEmitter->GetGrassAtlasTexture())
    {
        ...
        const auto texDesc = grassTex->GetD3D12ResourceDesc();
        texSrv.Format = texDesc.Format;
        texSrv.Texture2D.MipLevels = texDesc.MipLevels;
        texSrv.Texture2D.PlaneSlice = 0;
        grassTex->CreateShaderResourceView(&texSrv, &singleExpandedDrawDescriptors_, 0);
    }
    ...
    vertexSrv.Buffer.NumElements = emitterData.GrassCount * kMaxVerticesPerBlade;
    vertexSrv.Buffer.StructureByteStride = sizeof(GrassRenderVertex);
    vertexSrv.Buffer.Flags = D3D12_BUFFER_SRV_FLAG_NONE;
    singleExpandedVertexBuffer_->CreateShaderResourceView(&vertexSrv, &singleExpandedDrawDescriptors_, 1);
    ...
    counterSrv.Buffer.StructureByteStride = sizeof(uint32_t);
    counterSrv.Buffer.Flags = D3D12_BUFFER_SRV_FLAG_NONE;
    singleVisibleVertexCountBuffer_->CreateShaderResourceView(&counterSrv, &singleExpandedDrawDescriptors_, 2);
}
...
void CrossAdapterGrassEmitter::EnsureSingleExpandResourcesInitialized()
{
    if (singleExpandResourcesInitialized_)
        return;

    InitSingleExpandPSO();
    CreateSingleExpandBuffers();
    DescriptorInitializeSingleExpand();
    DescriptorInitializeSingleExpandedDraw();
    singleExpandResourcesInitialized_ = true;
    needRegenerate = true;
}
...
void CrossAdapterGrassEmitter::EnsureSingleWindFluidGpuInitialized()
{
    if (singleWindFluid_.IsInitialized())
        return;
    if (singleWindFluidInitGiveUp_)
        return;
...
void CrossAdapterGrassEmitter::EnsureSingleExpandWindVelocitySnapshot()
{
    auto bindVelocitySrv = [this](GResource& resource)
    {
        ...
        velSrv.Texture2D.MostDetailedMip = 0;
        velSrv.Texture2D.MipLevels = 1;
        resource.CreateShaderResourceView(&velSrv, &singleExpandDescriptors_, 1);
    };
    ...
    if (singleExpandWindVelFallback_)
    {
        bindVelocitySrv(*singleExpandWindVelFallback_);
        return;
    }
    ...
    singleExpandWindVelFallback_ = std::make_unique<GResource>(
        primeDevice,
        fallbackDesc,
        L"Single GPU Expand Wind Velocity Fallback",
        nullptr,
        D3D12_RESOURCE_STATE_COMMON);
...
void CrossAdapterGrassEmitter::Update()
{
    ...
    if (primeGrassEmitter && gameObject)
    {
        ...
        const uint32_t activeWindGrid = renderPath_ == RenderPath::SingleExpanded
                                            ? singleWindFluid_.GetGridResolution()
                                            : windFluid.GetGridResolution();
        UpdateWindFieldWorldParams(
            emitterData,
            cullData,
            static_cast<float>(std::max(activeWindGrid, 1u)));
    }

    // ��������� ��������� ��������� ��������
    GrassEmitterData primeEmitterDataCopy = emitterData;
    if (renderPath_ == RenderPath::SingleGeometry)
    {
    ...
    if (renderPath_ == RenderPath::SingleGeometry)
        ...
        primeGrassEmitter->UpdateObjectConstants();
    
    if (needRegenerate && renderPath_ == RenderPath::SingleGeometry)
    {
        GenerateGrassDataCPU();
...
{
    const bool multiExpanded = renderPath_ == RenderPath::MultiExpanded;
    const bool singleExpanded = renderPath_ == RenderPath::SingleExpanded;

    if (multiExpanded)
        EnsureSharedComputeResourcesInitialized();
    else if (singleExpanded)
        EnsureSingleExpandResourcesInitialized();
    ...
    if (!multiExpanded && !singleExpanded)
        return;

    {
        ...
        if (multiExpanded)
            ...
            EnsureSingleWindFluidGpuInitialized();

        auto& activeWindFluid = multiExpanded ? windFluid : singleWindFluid_;
        auto& activeEmitterCb = multiExpanded ? sharedGrassEmitterCb_ : singleGrassEmitterCb_;
        auto& activeCullCb = multiExpanded ? sharedGrassCullCb_ : singleGrassCullCb_;
        const auto& computeDevice = multiExpanded ? secondDevice : primeDevice;
        GBuffer* activeGrassBuffer = multiExpanded ? grassBuffer.get() : singleGrassBuffer_.get();
        GBuffer* activeExpandedVertexBuffer =
            multiExpanded ? expandedVertexBuffer.get() : singleExpandedVertexBuffer_.get();
        GBuffer* activeVisibleVertexCountBuffer =
            multiExpanded ? visibleVertexCountBuffer.get() : singleVisibleVertexCountBuffer_.get();
        GDescriptor* activeExpandDescriptors =
            multiExpanded ? &expandDescriptors : &singleExpandDescriptors_;
        const auto& activeExpandPSO = multiExpanded ? expandPSO : singleExpandPSO_;
        const auto& activeExpandRS = multiExpanded ? expandRS : singleExpandRS_;
        const auto& activeGeneratePSO = multiExpanded ? generatePSO : singleGeneratePSO_;
        const auto& activeComputeRS = multiExpanded ? computeRS : singleComputeRS_;
        GDescriptor* activeComputeDescriptors =
            multiExpanded ? &computeDescriptors : &singleComputeDescriptors_;
        ...
        if (!activeEmitterCb)
        {
            activeEmitterCb = std::make_shared<GBuffer>(
                computeDevice, grassEmitterCbBytes, 1,
                multiExpanded ? L"Multi GPU Grass Emitter Constant" : L"Single GPU Grass Emitter Constant");
            activeCullCb = std::make_shared<GBuffer>(
                computeDevice, grassCullCbBytes, 1,
                multiExpanded ? L"Multi GPU Grass Cull Constant" : L"Single GPU Grass Cull Constant");
        }
        const auto& constantBuffer = activeEmitterCb;
        const auto& cullBuffer = activeCullCb;

        const bool gpuFluidLive =
            activeWindFluid.IsInitialized() && (emitterData.WindFluidEnable >= 0.5f);
        ...
        UpdateWindFieldWorldParams(
            emitterData,
            cullData,
            static_cast<float>(std::max(activeWindFluid.GetGridResolution(), 1u)));
        ...
        wf.CellWorldSize =
            (emitterData.WindFieldWorldParams.z > 1e-5f ? (emitterData.WindFieldWorldParams.z * 2.f /
                                                             static_cast<float>(activeWindFluid.GetGridResolution()))
                                                         : emitterData.WorldSize / static_cast<float>(activeWindFluid.GetGridResolution()));
        ...
        if (needRegenerate)
        {
            cmdList->TransitionBarrier(activeGrassBuffer->GetD3D12Resource(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
            cmdList->FlushResourceBarriers();

            cmdList->SetPipelineState(*activeGeneratePSO.get());
            cmdList->SetRootSignature(*activeComputeRS);

            cmdList->SetDescriptorsHeap(activeComputeDescriptors);

            cmdList->SetRootConstantBufferView(0, *constantBuffer);
            cmdList->SetRootDescriptorTable(1, activeComputeDescriptors, 0);
            cmdList->Dispatch(threadGroups, 1, 1);

            cmdList->UAVBarrier(activeGrassBuffer->GetD3D12Resource());
            cmdList->FlushResourceBarriers();
        }

        uint32_t zeroCounter = 0;
        activeVisibleVertexCountBuffer->LoadData(&zeroCounter, cmdList);

        // Expand reads grass + velocity as SRV and writes expanded vertices as UAV.
        cmdList->TransitionBarrier(activeGrassBuffer->GetD3D12Resource(),
                                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        cmdList->TransitionBarrier(activeExpandedVertexBuffer->GetD3D12Resource(),
                                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        cmdList->TransitionBarrier(activeVisibleVertexCountBuffer->GetD3D12Resource(),
                                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        cmdList->FlushResourceBarriers();

        cmdList->SetPipelineState(*activeExpandPSO.get());
        cmdList->SetRootSignature(*activeExpandRS);
        cmdList->SetDescriptorsHeap(activeExpandDescriptors);
        cmdList->SetRootConstantBufferView(0, *constantBuffer);
        cmdList->SetRootConstantBufferView(1, *cullBuffer);
        cmdList->SetRootDescriptorTable(2, activeExpandDescriptors, 0);
        cmdList->SetRootDescriptorTable(3, activeExpandDescriptors, 2);
        cmdList->SetRootDescriptorTable(4, activeExpandDescriptors, 3);
        cmdList->Dispatch(threadGroups, 1, 1);

        cmdList->UAVBarrier(activeExpandedVertexBuffer->GetD3D12Resource());
        cmdList->FlushResourceBarriers();

        if (multiExpanded)
        {
            cmdList->CopyResource(crossAdapterExpandedVertexBuffer->GetSharedResource().GetD3D12Resource(),
                                  activeExpandedVertexBuffer->GetD3D12Resource());
            cmdList->CopyResource(crossAdapterVisibleVertexCountBuffer->GetSharedResource().GetD3D12Resource(),
                                  activeVisibleVertexCountBuffer->GetD3D12Resource());
...
void CrossAdapterGrassEmitter::SetRenderPath(const RenderPath path)
{
    if (renderPath_ == path)
        return;

    const bool wasMulti = renderPath_ == RenderPath::MultiExpanded;
    renderPath_ = path;
    if (renderPath_ == RenderPath::MultiExpanded)
    {
    ...
    if (renderPath_ == RenderPath::SingleExpanded)
        singleWindFluidInitGiveUp_ = false;

    // Every path owns different GPU buffers; refresh the active copy after a switch.
    needRegenerate = true;
}
